...
public class gameObject {
    ...
    protected float[] barycentricBuffer;
    ...
    protected Vector3f axisOfRotationRate;
    protected Quaternionf rotationRateRate;
    ...
    protected double rotationRateSpeed = 0;
    ...
    public gameObject(String modelName, String textureName) {
        ...
        modelLoader ml = new modelLoader("resources/models/" + modelName + ".obj");

        vertexBuffer = ml.getVertexBuffer();
        uvBuffer = ml.getUVBuffer();
        normalBuffer = ml.getNormalBuffer();
        barycentricBuffer = ml.getBarycentricBuffer();
        ...
        axisOfRotationRate = new Vector3f(0, 0, 0);
    }
...
//            r.set(r.x, 0, r.z);
        ...
        double thetaRate = dt * rotationRateSpeed;
        accumulator = new Vector3f();
        axisOfRotationRate.mul((float)Math.sin(thetaRate / 2.0), accumulator);
        rotationRateRate = new Quaternionf(accumulator.x, accumulator.y, accumulator.z,
                                            (float)Math.cos(thetaRate / 2.0));
        Matrix3f rotate = new Matrix3f();
        rotationRate.get(rotate);
        axisOfRotation.mul(rotate);
...
//        result.mul(translation);
    ...
    public float[] getBarycentricBuffer() { return barycentricBuffer; }
    ...
    public void setUniverse(gameUniverse universe) { this.universe = universe; }

    public void setThrust(Vector3f pointOnObject, Vector3f thrust) {

        // todo: account for thrust being 0 vector
        //  (this will probably result in div by 0 error if left unchecked)
        if (thrust.length() == 0) {
            rotationRateSpeed = 0;
        } else {

            pointOnObject.cross(thrust, axisOfRotationRate);
            axisOfRotationRate.normalize();

            double thrustScalar = thrust.length() / m;
            double speedProportion = axisOfRotationRate.dot(axisOfRotation);

            rotationSpeed += speedProportion * thrustScalar;
            rotationRateSpeed = (1 - speedProportion) * thrustScalar;
        }
